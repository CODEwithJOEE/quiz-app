...
export default function PendingDeletionCard({ student }: { student: any }) {
  const t = useTranslations("PendingDeletion");
  ...
  const deleteDateStr = deleteDate.toLocaleDateString(undefined, {
    month: "short",
    day: "numeric",
    year: "numeric",
  });
      ...
      >
        ...
        {isCritical ? (
          <>
            ...
            <span>
              <b>{t("deletesTomorrow")}</b> ({deleteDateStr})
            </span>
          </>
        ) : (
          <span>
      ...
      {confirming && (
        <div className="bg-card rounded-xl p-3 space-y-2 border border-border">
          <p className="text-xs font-medium">
            {t("restoreConfirm", { name: student.full_name })}
          </p>
